...
| **AgentTx provenance fail-closed on the durable CAS** (custom secondary tier `agenttx_cas` over `fs_python`): matching provenance loads KV into a fresh worker's attention (cached=2048); a MISMATCHED provenance (RoPE differs → vLLM's own block hashes still match) **FAILS CLOSED** at the AgentTx tier (cached=0 → recompute), so KV under a different model/config is never silently loaded | `agenttx/kv_cas_tier.py`, `phase11/kv_cas_provenance.py` |
...
- ~~Restore durable KV into a fresh vLLM worker's attention~~ **DONE — `phase11` (durable CAS tier; cross-process hit into attention, 1.45×@2K → 1.92×@8K).**
- ~~AgentTx provenance fail-closed verification on top of the CAS~~ **DONE — `phase11b` (`agenttx_cas` tier; provenance mismatch → fail-closed → recompute).** Remaining polish: turn-LSN-addressed manifests (the CAS is currently prefix/content-addressed).
